...
export default {
  ...
  methods: {
    ...
    uncollect() {
      this.$axios.post('/studentAnswer/deleteCollectQuestion',{"questionId":this.paperData.questionList[0][this.Index].questionId,"userStudentId":this.userInfo.id}).then((res) => {
        if(res.data.code == 200){
           this.$message({
            message: '已取消收藏',
            type: 'success'
